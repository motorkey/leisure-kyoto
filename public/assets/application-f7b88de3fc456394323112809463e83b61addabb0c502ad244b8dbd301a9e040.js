...
(function() {
  $(document).on('ready page:load', function() {
    var $calendar, activeDate, backgroundColor, colorActive, eventId;
    ...
    $.ajax({
      ...
      for (i = 0, len = data.length; i < len; i++) {
        x = data[i];
        results.push(activeDate.push(x.event_on));
      }
      return results;
    });
    return $calendar.fullCalendar({
      ...
      dayRender: function(date, cell) {
        ...
        if ($.inArray(isoDate, activeDate) !== -1) {
          return cell.css('background-color', colorActive);
        } else if (isoDate === moment().format('YYYY-MM-DD')) {
          return cell.css('background-color', backgroundColor);
        }
      },
      dayClick: function(date, jsEvent, view) {
        ...
        if ($day.css('background-color') === backgroundColor) {
          return $.ajax({
            ...
            return activeDate.push(isoDate);
          });
        } else {
          return $.ajax({
            ...
            return activeDate.splice($.inArray(isoDate, activeDate), 1);
          });
        }
      }
    });
      ...
      markers = handler.addMarkers([
        {
          ...
          'lng': 135,
          'picture': {
            'url': '/assets/map_pin_blue-4c1c3da322ff95520c0c66e9e3d63ecbf6ffcfcaec5a762f8ac2a6cc0cd75c44.jpg',
            'width': 36,
            'height': 36
          }
        }
